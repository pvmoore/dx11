...
public:
	...
	VertexShader getVS(const wstring& filename, string entry = "");
	HullShader getHS(const wstring& filename, string entry = "");
	DomainShader getDS(const wstring& filename, string entry = "");
	GeometryShader getGS(const wstring& filename, string entry = "");
	PixelShader getPS(const wstring& filename, string entry = "");
	ComputeShader getCS(const wstring& filename, string entry = "", D3D_SHADER_MACRO* defines=nullptr);
private:
	ComPtr<ID3DBlob> compile(const wstring& filename, string entry, string target, D3D_SHADER_MACRO* defines = nullptr);
};
